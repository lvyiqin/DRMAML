...
class ImageNetR(Dataset):
  def __init__(self, root_path, split='train', image_size=224,
    ...
    split_file = os.path.join(root_path, "ImageNetR_category_split_"+split_tag + '.pt')
...
@register('meta-imagenetr')
class MetaImageNetR(ImageNetR):
  def __init__(self, root_path, split='train', image_size=224,
               normalization=True, transform=None, val_transform=None,
               n_batch=200, n_episode=4, n_way=5, n_shot=1, n_query=15, n_pick=4):
    super(MetaImageNetR, self).__init__(root_path, split, image_size,
