...
class ImageNetC(Dataset):
  def __init__(self, root_path, split='train', image_size=224,
    ...
    split_file = os.path.join(root_path, "ImageNetC_category_split_"+split_tag + '.pt')
    # pdb.set_trace()
...
@register('meta-imagenetc')
class MetaImageNetC(ImageNetC):
  def __init__(self, root_path, split='train', image_size=224,
               normalization=True, transform=None, val_transform=None,
               n_batch=200, n_episode=4, n_way=5, n_shot=1, n_query=15, n_pick=4):
    super(MetaImageNetC, self).__init__(root_path, split, image_size,
